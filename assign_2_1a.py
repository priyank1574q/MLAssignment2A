...
len_inputs = len(x_train[0])
classes = len(list(set(y_train)))

max_val = float(np.max(x_train))

x_train = x_train[:,1:]/max_val
x_test = x_test[:,1:]/max_val

hidden_layer = []

for i in range(7, len(sys.argv)):
    hidden_layer.append(int(sys.argv[i]))
...
def layers(inputs = len_inputs, outputs = classes, hidden = hidden_layer):
    w = [None]*(len(hidden))
    z = [None]*(len(hidden))
